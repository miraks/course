...
firstRepeat ::
  ...
  -> Optional a
firstRepeat xs =
  eval (findM member xs) S.empty

member :: Ord a => a -> State (S.Set a) Bool
member x = State $ \s -> if S.member x s then (True, s) else (False, S.insert x s)
...
distinct ::
  ...
  -> List a
distinct =
  listh . S.toList . flip exec S.empty . filtering member
...
isHappy ::
  ...
  -> Bool
isHappy x =
  contains 1 $ eval (findM check (produce sumOfSquares x)) S.empty
  where
    check :: Integer -> State (S.Set Integer) Bool
    check n = State $ \s -> (n == 1 || S.member n s, S.insert n s)

    sumOfSquares :: Integer -> Integer
    sumOfSquares =
      toInteger . sum . map square . digits . fromInteger
      where
        digits :: Int -> List Int
